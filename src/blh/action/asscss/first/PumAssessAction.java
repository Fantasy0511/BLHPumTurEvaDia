...
public class PumAssessAction extends AbstractActionSupport {
	...
	public String execute() throws Exception {
		...
		List<Double> sonbarValue1 = Arrays.asList((double) pumpAssResult.getSwingAss().get(0).doubleValue(),
		...
		// 温度底层指标得分
		List<String> sonbarName2 = Arrays.asList("转子温度", "上、下迷宫环温度", "下导轴承温度", "定子线圈温度", "定子铁芯温度", "推力轴承发电温度", "上导轴承温度",
				"水导轴承温度");
		List<Double> sonbarValue2 = Arrays.asList((double) pumpAssResult.getGeneratorRotor(),
				(double) pumpAssResult.getLabyrinthRing(), (double) pumpAssResult.getLowerBearing(),
				(double) pumpAssResult.getStatorCoil(), (double) pumpAssResult.getStatorCore(),
				(double) pumpAssResult.getThrustBearing(), (double) pumpAssResult.getUpperBearing(),
		...
		assessView = new AssessView(pumSum, topRemark, pumpAssResult.getMainShaftSeal() + "",
				pumpAssResult.getSwingAss().get(6) + "", pumpAssResult.getHistory() + "",
