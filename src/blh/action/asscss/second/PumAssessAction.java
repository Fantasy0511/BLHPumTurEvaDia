...
public class PumAssessAction extends AbstractActionSupport {
	...
	public String execute() throws Exception {
		...
		String[] category = { "主轴密封状态", "", "", "", "振动状态", "", "", "", "", "", "", "", "", "", "温度状态", "", "", "", "",
				"", "", "" };
		String[] names = { "供水流量", "供水温度", "过滤器前压力", "过滤器后压力", "主轴X轴摆动跳闸 ", "主轴Y轴摆动跳闸 ", "主轴X轴位移", "主轴Y轴位移",
				"水导轴承X轴摆动跳闸 ", "水导轴承Y轴摆动跳闸", "水导轴承Z轴摆动跳闸", "水导轴承X轴摆动速率", "水导轴承Y轴摆动速率", "水导轴承Z轴摆动速率", "上导冷却水出水温度",
				"上导轴瓦温度", "上导油槽温度", "下导冷却水出水温度", "下导轴瓦温度", "下导油槽温度" };
		double[] values = { (double) pumpAssResult.getWaterFlow(), (double) pumpAssResult.getWaterTemperature(),
				...
				pumpAssResult.getSwingAss().get(6).doubleValue(), pumpAssResult.getSwingAss().get(7).doubleValue(),
				pumpAssResult.getSwingAss().get(8).doubleValue(), pumpAssResult.getSwingAss().get(9).doubleValue(),

				(double) pumpAssResult.getUpperWater(), (double) pumpAssResult.getUpperBearing(),
				(double) pumpAssResult.getUpperOil(), (double) pumpAssResult.getLowerWater(),
				(double) pumpAssResult.getLowerBearing(), (double) pumpAssResult.getLowerOil() };
		...
		List<Double> barValue = Arrays.asList((double) pumpAssResult.getMainShaftSeal(),
				(double) pumpAssResult.getSwingAss().get(10), (double) pumpAssResult.getHistory(),
		...
		List<Double> sonbarValue = Arrays.asList((double) pumpAssResult.getWaterFlow(),
				(double) pumpAssResult.getWaterTemperature(), (double) pumpAssResult.getBeforeStress(),
		...
		// 振动底层指标得分
		List<String> sonbarName1 = Arrays.asList("主轴X轴摆动跳闸 ", "主轴Y轴摆动跳闸 ", "主轴X轴位移", "主轴Y轴位移", "水导轴承X轴摆动跳闸 ",
				"水导轴承Y轴摆动跳闸", "水导轴承Z轴摆动跳闸", "水导轴承X轴摆动速率", "水导轴承Y轴摆动速率", "水导轴承Z轴摆动速率");
		List<Double> sonbarValue1 = Arrays.asList(pumpAssResult.getSwingAss().get(0).doubleValue(),
				pumpAssResult.getSwingAss().get(1).doubleValue(), pumpAssResult.getSwingAss().get(2).doubleValue(),
				pumpAssResult.getSwingAss().get(3).doubleValue(), pumpAssResult.getSwingAss().get(4).doubleValue(),
				pumpAssResult.getSwingAss().get(5).doubleValue(), pumpAssResult.getSwingAss().get(6).doubleValue(),
				pumpAssResult.getSwingAss().get(7).doubleValue(), pumpAssResult.getSwingAss().get(8).doubleValue(),
		...
		// 温度底层指标得分
		List<String> sonbarName2 = Arrays.asList("上导冷却水出水温度", "上导轴瓦温度", "上导油槽温度", "下导冷却水出水温度", "下导轴瓦温度", "下导油槽温度");
		List<Double> sonbarValue2 = Arrays.asList((double) pumpAssResult.getUpperWater(),
				(double) pumpAssResult.getUpperBearing(), (double) pumpAssResult.getUpperOil(),
				(double) pumpAssResult.getLowerWater(), (double) pumpAssResult.getLowerBearing(),
				(double) pumpAssResult.getLowerOil());
		List<Double> sonbarValueRatio2 = Arrays.asList(0.16, 0.16, 0.16, 0.16, 0.16, 0.16);
		BarData temperatureBar = BarData.create("温度底层指标得分", "", "性能状态", "得分", sonbarName2, sonbarValue2);
		...
		sonAssessView = new SonAssessView(mainShaftSealBar, mainShaftSealPie, SingleBar, SinglePie, temperatureBar,
				temperaturePie);
