...
public class SvmTrain {
	...
	public static svm_model main(String argv[], Vector<svm_node[]> vx,
			Vector<Double> vy) throws IOException {
		SvmTrain t = new SvmTrain();
		svm_model model = t.run(argv, vx, vy);
		return model;
	}

	private svm_model run(String argv[], Vector<svm_node[]> vx,
			Vector<Double> vy) throws IOException {
		svm_model thismodel = new svm_model();
		...
		if (cross_validation != 0) {
			...
			thismodel = svm.svm_train(prob, param);
		}
		return thismodel;
	}
	...
	private void read_problem(Vector<svm_node[]> vx, Vector<Double> vy)
			throws IOException {
		int max_index = vx.get(0).length;
