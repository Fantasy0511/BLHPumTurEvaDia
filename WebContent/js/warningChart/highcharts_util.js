 ...
 * @returns
 */
function _drawLineStepChart(title, xName, yName, data, id, callback) {
	var options = {
		chart : {
			zoomType : 'x',
			spacingRight : 20,
 ...
 *            图div容器的id
 */
function _drawLine(lineData, id,type) {
	Highcharts.setOptions({
		global: {
			useUTC: false
		}
	...
	if(type.indexOf("bool") != -1){
		var stepData=lineData.data[0];
		stepData.step='center';
		var step1= [];
		step1.push(stepData);
		_drawLineStepChart(lineData.title, lineData.xName, lineData.yName,
				step1, id);
	}else{
			...
			.highcharts(
					{
						...
						tooltip : {
							...
							pointFormat : '' + '',
							footerFormat : '<table><tbody><tr><td style="color:{series.color};padding:0">故障概率: </td><td style="padding:0"><b>{point.y:.3f}</b></td></tr></tbody></table>',
							shared : true,
							useHTML : true
						},
						plotOptions : {
 ...
 *            图的数据
 */
function drawPic(id, data,types) {
	if ((!data) || (data == {})) {
		var div = $("#" + id);
		div.empty();
