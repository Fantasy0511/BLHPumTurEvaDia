...
public class ExcAssessAction extends AbstractActionSupport {
	...
	public String execute() throws Exception {
		...
		/** 评估主页面数据获取 */
		// 详细评估信息
		String[] category = { "励磁故障信号", "", "", "机组监测电气量", "", "", "发电机故障信号", "", "", "励磁发电系统温度", "", "", "灭磁开关动作状态",
				"" };
		String[] names = { "励磁电源故障", "电压/频率跳闸", "晶闸管桥风扇故障", "有功功率测量值", "励磁电压a", "机组频率", "转子绝缘报警次数", "发电机停止报警次数",
				"频率保护报警", "发电机热风温度", "发电机冷风温度", "空冷器出水温度", "励磁电流", "励磁电压b" };
		...
		// 返回评估结果
		// 返回评估结果
		assessView = new AssessView(excSum, topRemark, excAssResult.getIndicator().get(1).toString(),
				excAssResult.getIndicator().get(2).toString(), excAssResult.getIndicator().get(3).toString(),
				excAssResult.getIndicator().get(4).toString(), excAssResult.getIndicator().get(5).toString(),
		...
		// 机组电气量状态
		List<String> sonBarName2 = Arrays.asList("有功功率测量值", "励磁电压", "机组频率");
		List<Double> sonbarValue2 = Arrays.asList(excAssResult.getIndicator().get(9).doubleValue(),
				excAssResult.getIndicator().get(10).doubleValue(), excAssResult.getIndicator().get(11).doubleValue());
		BarData sonBar2 = BarData.create("机组监测电气量评估指标", "", "性能状态", "得分", sonBarName2, sonbarValue2);
		...
		List<Double> sonbarValue3 = Arrays.asList(excAssResult.getIndicator().get(12).doubleValue(),
				excAssResult.getIndicator().get(13).doubleValue(), excAssResult.getIndicator().get(14).doubleValue());
		...
		List<Double> sonbarValue4 = Arrays.asList(excAssResult.getIndicator().get(15).doubleValue(),
				excAssResult.getIndicator().get(16).doubleValue(), excAssResult.getIndicator().get(17).doubleValue());
		BarData sonBar4 = BarData.create("励磁发电系统温度评估指标", "", "性能状态", "得分", sonBarName2, sonbarValue2);
		...
		List<Double> sonbarValue5 = Arrays.asList(excAssResult.getIndicator().get(18).doubleValue(),
				excAssResult.getIndicator().get(19).doubleValue());
		BarData sonBar5 = BarData.create("灭磁开关动作状态评估指标", "", "性能状态", "得分", sonBarName2, sonbarValue2);
		PieData sonPie5 = PieData.create("灭磁开关动作状态评估指标得分", sonBarName5, sonbarValue5, "得分XXX");

		sonAssessView = new SonAssessView(sonBar1, sonPie1, sonBar2, sonPie2, sonBar3, sonPie3, sonBar4, sonPie4,
				sonBar5, sonPie5);
